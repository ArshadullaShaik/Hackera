window.HackeraCardUtils = (() => {
  const cardDateOptions = { month: "short", day: "numeric", timeZone: "UTC" };
  ...
  function extractPrizeFromArray(prizes) {
    ...
    for (const prize of prizes) {
      ...
      if (prize && typeof prize === "object") {
        for (const field of [prize.cash, prize.amount, prize.value, prize.prize, prize.title, prize.name, prize.description]) {
  ...
  function resolvePrizeText(item) {
    ...
    if (prizePoolMatch && prizePoolMatch[1].trim()) {
  ...
  function resolveTrackBadges(item) {
    ...
    if (/\b(ai|ml|machine learning|artificial intelligence|llm|genai|gpt|deep learning|neural|nlp|computer vision|chatgpt)\b/i.test(combined)) trackBadges.push("AI / ML");
    if (/\b(web3|crypto|blockchain|ethereum|solana|defi|nft|smart contract|dao|bitcoin|dapp)\b/i.test(combined)) trackBadges.push("Web3 / Blockchain");
    if (/\b(mobile|ios|android|flutter|react native|app dev|app building)\b/i.test(combined)) trackBadges.push("Mobile");
    if (/\b(cyber|security|ctf|privacy|cryptography|infosec|hacking)\b/i.test(combined)) trackBadges.push("Cybersecurity");
    if (/\b(fintech|finance|payment|banking|trading|quant|defi)\b/i.test(combined)) trackBadges.push("Fintech");
    if (/\b(health|bio|medical|healthcare|medtech)\b/i.test(combined)) trackBadges.push("Healthtech");

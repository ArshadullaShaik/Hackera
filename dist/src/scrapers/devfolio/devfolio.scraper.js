    ...
    async scrape() {
        ...
        try {
            ...
            if (hackathonData.props?.pageProps?.dehydratedState?.queries?.[0]?.state?.data) {
                ...
                logger.debug({ hackathonCount: hackathons.length }, "Extracted hackathons from dehydrated state");
            }
                ...
                logger.debug({ hackathonCount: hackathons.length }, "Extracted hackathons from top-level keys");
            }
            if (hackathons.length === 0) {
                logger.error({ dataKeys: Object.keys(hackathonData) }, "Found JSON but no hackathon arrays");
                throw new Error("Devfolio data found but no hackathon arrays in expected location");
            }
            // Validate individual hackathons
            const envelopeData = { open_hackathons: hackathons };
            logger.info({ eventCount: hackathons.length }, "Devfolio hackathons found");
            // Process individual hackathons (fail-soft)
            let processedCount = 0;
            for (const rawHackathon of hackathons) {
                if (processedCount >= this.MAX_EVENTS) {
                    logger.info({ maxEvents: this.MAX_EVENTS }, "Reached maximum event limit for Phase 2");
                    break;
                }
                try {
            ...
            logger.error({ error: error instanceof Error ? error.message : String(error) }, "Devfolio scraper failed at envelope level");
            throw error;
        }
    }

...
async function processJob(job) {
    ...
    try {
        ...
        logger.info({ jobId: job.id, platform, created: result.created, updated: result.updated }, "Scrape job completed");
        return result;
    }
    catch (error) {
...
async function main() {
    ...
    worker.on("failed", (job, err) => {
        logger.error({ jobId: job?.id, name: job?.name, error: err.message, attempt: job?.attemptsMade }, "Job failed");
    });

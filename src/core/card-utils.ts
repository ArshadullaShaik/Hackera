...
function sanitizePrizeValue(value: unknown): string {
  return String(value ?? "").replace(/<[^>]*>?/gm, "").trim();
}

function extractPrizeFromArray(prizes: unknown): string | null {
...
export function resolveTrackBadges(item: CardHackathonItem): string[] {
  const rawDescription = item.description || "";
  const trackBadges: string[] = [];

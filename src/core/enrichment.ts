 ...
 */
export function extractPrizePool(rawPayload: any = {}, textContent: string = ""): string {
  ...
  if (rawPayload?.prize_money) {
    ...
    if (clean) return clean.startsWith("$") || clean.startsWith("₹") ? clean : `₹${clean}`;
  }
  ...
  if (Array.isArray(rawPayload?.prizes) && rawPayload.prizes.length > 0) {
    ...
    if (first.cash) return String(first.cash);
    if (first.amount) return `$${first.amount}`;
  }
...
export function formatDescription(
  ...
  tracks: string[] = [],
  prizePool: string = "Prizes Available"
): string {

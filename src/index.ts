  ...
  try {
    ...
    for (const { name, scraper } of scrapers) {
      try {
        ...
        logger.info(
          { platform: name, count: hackathons.length },
    ...
    for (const { platform, events } of allHackathons) {
      ...
      logger.info(
        { platform, count: activeEvents.length },
        `Persisting ${activeEvents.length} active events from ${platform}`
      );
    ...
    console.log(
      `✓ PostgreSQL persistence working: ${totalCreated + totalUpdated > 0 ? "YES" : "NO"}`
    );
    console.log(
      `✓ Upsert logic (no duplicates on second run): ${totalUpdated > 0 ? "YES (had updates)" : "UNKNOWN (run twice to verify)"}`
    );
    console.log("=".repeat(80) + "\n");
  } catch (error) {
    logger.error(
